...
try:
    ...
    raise SystemExit(
        "Falta psutil. Ejecuta: python -m pip install psutil"
    ) from exc
...
def gpu_status() -> dict[str, str]:
    ...
    try:
        usage, used, total, temp = [
            part.strip()
            for part in output.splitlines()[0].split(",")
        ]
        return {
            "usage": usage,
            "used": used,
...
def disk_lines() -> list[str]:
    ...
    for part in psutil.disk_partitions(all=False):
        ...
        lines.append(
            (
                f"DISCO {mount[:2]}  "
                f"{usage.percent:>4.0f}%  "
                f"{usage.free / 1024**3:>5.0f} GB libres"
            )
        )
...
def snapshot() -> tuple[str, dict]:
    ...
    lines = [
        ...
        (
            ...
            f"{data['ram_used_gb']:.1f}/"
            f"{data['ram_total_gb']:.1f} GB"
        ),
    ]
...
class Overlay:
    def __init__(self) -> None:
        ...
        self.root.after(250, self._place_right)
        self.update()
    ...
    def _place_right(self) -> None:
        ...
        x = max(
            0,
            screen_width - WINDOW_WIDTH - MARGIN_RIGHT,
        )
    ...
    def _send_behind_app_windows(self) -> None:
        """Mantiene el monitor sobre el fondo, debajo de aplicaciones."""
        if os.name != "nt":
            return
