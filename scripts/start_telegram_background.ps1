...
"$(Get-Date -Format o) Bootstrap iniciado. Esperando proyecto: $ProjectRoot" |
    Add-Content -Encoding UTF8 -Path $BootstrapLog

$deadline = (Get-Date).AddSeconds(
    [Math]::Max(60, $WaitForProjectSeconds)
)

while (-not (Test-Path $Supervisor)) {
    if ((Get-Date) -ge $deadline) {
        "$(Get-Date -Format o) Proyecto no disponible tras la espera: $ProjectRoot" |
            Add-Content -Encoding UTF8 -Path $BootstrapLog
        exit 10
    }
...
if (Test-Path $PythonPathFile) {
    $candidate = (
        Get-Content $PythonPathFile -Raw
    ).Trim()
...
if (-not $python) {
    "$(Get-Date -Format o) No se encontró Python." |
        Add-Content -Encoding UTF8 -Path $BootstrapLog
    exit 11
}

"$(Get-Date -Format o) Proyecto disponible. Iniciando supervisor con $python." |
    Add-Content -Encoding UTF8 -Path $BootstrapLog

"$(Get-Date -Format o) Iniciando supervisor de Atlas Telegram." |
    Add-Content -Encoding UTF8 -Path $LogPath

& $python $Supervisor *>> $LogPath
$exitCode = $LASTEXITCODE

"$(Get-Date -Format o) El supervisor terminó con código $exitCode." |
    Add-Content -Encoding UTF8 -Path $LogPath

"$(Get-Date -Format o) Supervisor terminado con código $exitCode." |
    Add-Content -Encoding UTF8 -Path $BootstrapLog
