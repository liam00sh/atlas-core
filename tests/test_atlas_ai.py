...
class AtlasAIMixinTests(unittest.TestCase):
    def test_normalizes_context_user(self):
        self.assertEqual(AtlasAIMixin._normalize_ai_context_user("  Alex  "), "alex")
    ...
    def test_current_conversation_user_prefers_interlocutor(self):
        ...
        atlas.conversation_identity.get_conversation_owner.return_value = "Vega"
        atlas.get_user = Mock(return_value="Alex")
        self.assertEqual(atlas._get_current_conversation_user(), "Vega")
    ...
    def test_falls_back_to_authenticated_user(self):
        ...
        atlas.get_user = Mock(return_value="Alex")
        self.assertEqual(atlas._get_current_conversation_user(), "Alex")

    def test_permissions_use_permission_viewer_not_session_owner(self):
        atlas = object.__new__(AtlasAIMixin)
        atlas.conversation_identity = Mock()
        atlas.conversation_identity.get_permission_viewer.return_value = "Vega"
        atlas.users = Mock()
        atlas.users.get_profile.return_value = {"roles": []}
        atlas.get_user = Mock(return_value="Alex")
        self.assertFalse(atlas.can_manage_user_contexts())
        atlas.users.get_profile.assert_called_once_with("Vega")
