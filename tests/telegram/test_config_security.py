...
VALID_TOKEN = "123456:ABCDEFGHIJKLMNOPQRSTUVWXYZ_abcd"  # privacy-scan: allow
...
def test_invalid_token_error_does_not_echo_secret():
    secret = "not-a-valid-secret-token"  # privacy-scan: allow
    with pytest.raises(TelegramConfigError) as caught:
...
def test_audit_anonymizes_ids_and_omits_content(tmp_path):
    ...
    audit.record(action="message", result="ok", telegram_user_id="123", chat_id="456", atlas_user_id="Alex")
