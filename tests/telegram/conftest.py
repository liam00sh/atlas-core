...
def config(tmp_path):
    return TelegramConfig(
        enabled=True,
        token="123456:ABCDEFGHIJKLMNOPQRSTUVWXYZ_abcd",  # privacy-scan: allow
        data_dir=tmp_path,
...
def link_user(linker, *, user_id="100", chat_id="200", atlas_user="Alex"):
    user = TelegramUser(user_id, chat_id, username=atlas_user.casefold())
    code = linker.request_code(user)
    linker.confirm_code(code, atlas_user, user_exists=lambda candidate: candidate in {"Alex", "Vega"})
    return code
