...
class PersonTests(unittest.TestCase):
    def test_normalizes_name_aliases_and_summary(self):
        person = Person(
            name="  Alex Romero  ",
            aliases=["Alex", " Alex ", "", "AliasAnterior"],
            summary="  Usuario principal  ",
        )
        self.assertEqual(person.name, "Alex Romero")
        self.assertEqual(person.aliases, ["Alex", "AliasAnterior"])
        self.assertEqual(person.summary, "Usuario principal")
        self.assertTrue(person.matches_name("Alex"))
        self.assertTrue(person.matches_name("AliasAnterior"))

    def test_rejects_invalid_data(self):
        with self.assertRaises(ValueError):
            Person(name="")
        with self.assertRaises(ValueError):
            Person(name="Alex", status="invalid")
        with self.assertRaises(ValueError):
            Person(name="Alex", encounter_count=-1)
        with self.assertRaises(ValueError):
            Person(name="Alex", status=USER)

    def test_alias_encounters_and_serialization(self):
        person = Person(name="Vega", status=GUEST)
        self.assertTrue(person.add_alias("Zoe"))
        self.assertFalse(person.add_alias("Zoe"))
    ...
    def test_linking_user_profile_sets_user_status(self):
        person = Person(name="Vega")
        person.link_user_profile("Vega")
        self.assertTrue(person.is_user())
        self.assertEqual(person.status, USER)
        self.assertEqual(person.user_profile, "Vega")
