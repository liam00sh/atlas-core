...
def test_repeated_identity_load_uses_cache_and_returns_independent_objects(tmp_path, monkeypatch):
    ...
    storage.save_people([Person(name="REDACTED_2c7b6821719d")])
    calls = {"count": 0}
    ...
    assert second[0].name == "REDACTED_2c7b6821719d"
    assert storage.load_people()[0].name == "REDACTED_2c7b6821719d"
...
def test_save_invalidates_identity_cache(tmp_path):
    ...
    storage.save_people([Person(name="REDACTED_bc04a68d9192")])

    assert storage.people_file not in storage._entity_cache
    assert storage.load_people()[0].name == "REDACTED_bc04a68d9192"
...
def test_cache_is_scoped_to_storage_instance(tmp_path):
    ...
    first.save_people([Person(name="REDACTED_2c7b6821719d")])
    second.save_people([Person(name="REDACTED_bc04a68d9192")])

    assert first.load_people()[0].name == "REDACTED_2c7b6821719d"
    assert second.load_people()[0].name == "REDACTED_bc04a68d9192"
