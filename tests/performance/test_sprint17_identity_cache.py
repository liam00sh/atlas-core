...
def test_repeated_identity_load_uses_cache_and_returns_independent_objects(tmp_path, monkeypatch):
    ...
    storage.save_people([Person(name="Alex")])
    calls = {"count": 0}
    ...
    assert second[0].name == "Alex"
    assert storage.load_people()[0].name == "Alex"
...
def test_save_invalidates_identity_cache(tmp_path):
    ...
    storage.save_people([Person(name="Vega")])

    assert storage.people_file not in storage._entity_cache
    assert storage.load_people()[0].name == "Vega"
...
def test_cache_is_scoped_to_storage_instance(tmp_path):
    ...
    first.save_people([Person(name="Alex")])
    second.save_people([Person(name="Vega")])

    assert first.load_people()[0].name == "Alex"
    assert second.load_people()[0].name == "Vega"
