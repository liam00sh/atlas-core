...
def test_population_query_extracts_exact_place_name():
    assert _extract_entity_query("Cuántos habitantes tiene VillaEjemplo") == "VillaEjemplo"
    assert _extract_entity_query("población actual de Campo de Mirra") == "Campo de Mirra"


def test_progress_classification():
    assert classify_progress("Busca en Internet cuántos habitantes tiene VillaEjemplo") == "internet"
...
def test_progress_messages_are_personality_aware():
    daxter = build_progress_message("Busca en Internet VillaEjemplo", "Daxter")
    coco = build_progress_message("Busca en Internet VillaEjemplo", "Coco")
    assert daxter
    assert coco
    assert daxter != coco
