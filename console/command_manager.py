...
def load_commands():
    ...
    for module in pkgutil.iter_modules(
        ...
        # Algunos módulos del paquete commands son utilidades internas
        # compartidas y no comandos ejecutables. Solo registramos módulos
        # que expongan explícitamente un diccionario COMMAND válido.
        command_metadata = getattr(command, "COMMAND", None)
        if not isinstance(command_metadata, dict):
            continue
        if not str(command_metadata.get("name") or "").strip():
            continue
        ...
        # no tenga alias.
        for alias in command_metadata.get(
            "aliases",
            [],
        ):
